					...
					<rencontres>
						<domicile>
							<xsl:for-each select="//rencontre">
								...
								<xsl:variable name="numDayDom" as="xs:integer" select="../@num"/>
								<xsl:if test="$clubID = $clubReceveur">
									<rencontre num="{$numDayDom}">
										<clubInvité>
						...
						<exterieur>
							<xsl:for-each select="//rencontre">
								...
								<xsl:variable name="numDayExt" as="xs:integer" select="../@num"/>
								<xsl:if test="$clubID = $clubInvite">
									<rencontre num="{$numDayExt}">
										<clubReceveur>
